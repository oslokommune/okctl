...
//nolint:funlen
func TestIdentityManagerReconciler(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			meta := generateTestMeta(tc.withPurge, v1alpha1.ClusterIntegrations{Cognito: tc.withComponentFlag})

			state := &clientCore.StateHandlers{
				Cluster:         &mockClusterState{exists: tc.withDependenciesMet},
				IdentityManager: &mockIdentityManagerState{existingIdentityPool: tc.withComponentExists},
			...
			reconciler := NewIdentityManagerReconciler(&mockIdentityManagerService{
				createIdentityPoolBump: func() { creations++ },
				deleteIdentityPoolBump: func() { deletions++ },
			})
