...
func (s *blockstorageStore) RemoveBlockstorage(_ api.ID) (*store.Report, error) {
...
func (s *blockstorageStore) SaveBlockstorage(e *client.Blockstorage) (*store.Report, error) {
	...
	report, err := store.NewFileSystem(s.policy.BaseDir, s.fs).
		...
		Do()
	if err != nil {
		return nil, fmt.Errorf("storing blockstorage: %w", err)
	}

	return report, nil
}

// NewBlockstorageStore returns an initialised store
func NewBlockstorageStore(policy, serviceAccount, chart Paths, fs *afero.Afero) client.BlockstorageStore {
