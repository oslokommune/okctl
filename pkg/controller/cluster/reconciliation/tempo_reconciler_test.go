...
//nolint:funlen
func TestTempoReconciler(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			meta := generateTestMeta(tc.withPurge, v1alpha1.ClusterIntegrations{Tempo: tc.withComponentFlag})

			state := &clientCore.StateHandlers{
				Cluster: &mockClusterState{exists: tc.withDependenciesMet},
				Tempo:   &mockTempoState{exists: tc.withComponentExists},
			}

			reconciler := NewTempoReconciler(&mockMonitoringService{
				createTempoBump: func() { creations++ },
				deleteTempoBump: func() { deletions++ },
			})
...
type mockTempoState struct {
	exists bool
}

func (m mockTempoState) HasTempo() (bool, error) {
	return m.exists, nil
}
