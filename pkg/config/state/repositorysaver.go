...
type ContainerRepositorer interface {
	...
	DeleteContainerRepository(imageName string) (*store.Report, error)
}
...
func (r *repository) GetContainerRepositories() map[string]ContainerRepository {
	cluster := r.GetCluster()

	if cluster.Repositories == nil {
		cluster.Repositories = map[string]ContainerRepository{}
	}
...
func (r *repository) GetContainerRepository(imageName string) ContainerRepository {
	repositories := r.GetContainerRepositories()

	return repositories[imageName]
}

func (r *repository) SaveContainerRepository(imageName string, repository ContainerRepository) (*store.Report, error) {
	_ = r.GetContainerRepositories()

	r.state.Clusters[r.env].Repositories[imageName] = repository

	return r.save()
}

func (r *repository) DeleteContainerRepository(imageName string) (*store.Report, error) {
	cluster := r.GetCluster()

	delete(cluster.Repositories, imageName)
