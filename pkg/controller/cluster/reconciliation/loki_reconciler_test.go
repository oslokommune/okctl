...
//nolint:funlen
func TestLokiReconciler(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			meta := generateTestMeta(tc.withPurge, v1alpha1.ClusterIntegrations{Loki: tc.withComponentFlag})

			state := &clientCore.StateHandlers{
				Cluster: &mockClusterState{exists: tc.withDependenciesMet},
				Loki:    &mockLokiState{exists: tc.withComponentExists},
			}

			reconciler := NewLokiReconciler(&mockMonitoringService{
				createLokiBump: func() { creations++ },
				deleteLokiBump: func() { deletions++ },
			})
...
type mockLokiState struct {
	exists bool
}

func (m mockLokiState) HasLoki() (bool, error) {
	return m.exists, nil
}
