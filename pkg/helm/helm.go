...
// ExternalSecretsValues maps up the values.yaml file
// nolint: maligned
type ExternalSecretsValues struct {
	CRDs                          ExternalSecretsCRDs            `yaml:"crds"`
	CustomResourceManagerDisabled bool                           `yaml:"customResourceManagerDisabled"`
	Env                           ExternalSecretsEnv             `yaml:"env"`
	Image                         ExternalSecretsImage           `yaml:"image"`
	RBAC                          ExternalSecretsRBAC            `yaml:"rbac"`
	ReplicaCount                  int                            `yaml:"replicaCount"`
	Resources                     ExternalSecretsResources       `yaml:"resources"`
	SecurityContext               ExternalSecretsSecurityContext `yaml:"securityContext"`
	ServiceAccount                ExternalSecretsServiceAccount  `yaml:"serviceAccount"`
	ServiceMonitor                ExternalSecretsServiceMonitor  `yaml:"serviceMonitor"`
}

// ExternalSecretsCRDs determines how CRDs are created
type ExternalSecretsCRDs struct {
	Create bool `yaml:"create"`
}

// ExternalSecretsEnv sets the pod environment variables
type ExternalSecretsEnv struct {
...
// ExternalSecretsRBAC determines whether RBAC resources
// are created or not
type ExternalSecretsRBAC struct {
	Create bool `yaml:"create"`
}

// ExternalSecretsServiceAccount determines how service
// accounts are created
type ExternalSecretsServiceAccount struct {
	Create bool   `yaml:"create"`
	Name   string `yaml:"name"`
}

// ExternalSecretsImage set what container image
// to use
type ExternalSecretsImage struct {
	Repository string `yaml:"repository"`
	Tag        string `yaml:"tag"`
	PullPolicy string `yaml:"pullPolicy"`
}

// ExternalSecretsSecurityContext sets the pod security context
type ExternalSecretsSecurityContext struct {
	RunAsNonRoot bool `yaml:"runAsNonRoot"`
	FsGroup      int  `yaml:"fsGroup"`
}

// ExternalSecretsResources set the resource consumption of the pod
type ExternalSecretsResources struct {
	Limits   ExternalSecretsResourceEntry `yaml:"limits"`
	Requests ExternalSecretsResourceEntry `yaml:"requests"`
}

// ExternalSecretsResourceEntry sets the values for cpu and memory
type ExternalSecretsResourceEntry struct {
	CPU    string `yaml:"cpu"`
	Memory string `yaml:"memory"`
}

// ExternalSecretsServiceMonitor determines if the service monitor
// is enabled or not
type ExternalSecretsServiceMonitor struct {
	Enabled   bool   `yaml:"enabled"`
...
func ExternalSecrets(values interface{}) *Chart {
	return &Chart{
...
// for the helm chart
func DefaultExternalSecretsValues() *ExternalSecretsValues {
	return &ExternalSecretsValues{
		CustomResourceManagerDisabled: true,
		CRDs: ExternalSecretsCRDs{
			Create: false,
		},
		Env: ExternalSecretsEnv{
			AwsRegion:                  "eu-west-1",
		...
		RBAC: ExternalSecretsRBAC{
			Create: true,
		},
		ServiceAccount: ExternalSecretsServiceAccount{
			Create: false,
			Name:   "external-secrets", // This is fragile, should make these consts somewhere
		},
		ReplicaCount: 1,
		Image: ExternalSecretsImage{
			Repository: "godaddy/kubernetes-external-secrets",
			Tag:        "5.2.0",
			PullPolicy: "IfNotPresent",
		},
		SecurityContext: ExternalSecretsSecurityContext{
			RunAsNonRoot: true,
			FsGroup:      65534, // nolint: gomnd
		},
		Resources: ExternalSecretsResources{
			Limits: ExternalSecretsResourceEntry{
				CPU:    "100m",
				Memory: "128Mi",
			},
			Requests: ExternalSecretsResourceEntry{
				CPU:    "100m",
				Memory: "128Mi",
			},
		},
		ServiceMonitor: ExternalSecretsServiceMonitor{
			Enabled:   false,
			Interval:  "30s",
			Namespace: "",
		},
